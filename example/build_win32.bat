...
@if errorlevel 1 (
	...
	@if exist "%VSWHERE%" (
		@for /f "usebackq delims=" %%I in (`"%VSWHERE%" -latest -products * -requires Microsoft.VisualStudio.Component.VC.Tools.x86.x64 -property installationPath`) do @call "%%I\VC\Auxiliary\Build\vcvars32.bat" >nul
	)
	@if exist "%ProgramFiles%\Microsoft Visual Studio\2022\Community\VC\Auxiliary\Build\vcvars32.bat" @call "%ProgramFiles%\Microsoft Visual Studio\2022\Community\VC\Auxiliary\Build\vcvars32.bat" >nul
	@if exist "%ProgramFiles%\Microsoft Visual Studio\2022\Professional\VC\Auxiliary\Build\vcvars32.bat" @call "%ProgramFiles%\Microsoft Visual Studio\2022\Professional\VC\Auxiliary\Build\vcvars32.bat" >nul
	@if exist "%ProgramFiles%\Microsoft Visual Studio\2022\Enterprise\VC\Auxiliary\Build\vcvars32.bat" @call "%ProgramFiles%\Microsoft Visual Studio\2022\Enterprise\VC\Auxiliary\Build\vcvars32.bat" >nul
	@if exist "%ProgramFiles%\Microsoft Visual Studio\2022\BuildTools\VC\Auxiliary\Build\vcvars32.bat" @call "%ProgramFiles%\Microsoft Visual Studio\2022\BuildTools\VC\Auxiliary\Build\vcvars32.bat" >nul
)
...
@if not exist "%SDL3_LIB_DIR%\SDL3.lib" set SDL3_LIB_DIR=%SDL3_DIR%\build-win32\Release
@if not exist "%SDL3_LIB_DIR%\SDL3.lib" set SDL3_LIB_DIR=%SDL3_DIR%\build-win32\RelWithDebInfo
@if not exist "%SDL3_LIB_DIR%\SDL3.lib" (
	@echo SDL3.lib not found.
	@echo Build SDL first, for example:
	@echo   cmake -S "%SDL3_DIR%" -B "%SDL3_DIR%\build" -A Win32
